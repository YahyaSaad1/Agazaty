...
import { faIdCard, faPenToSquare, faTable, faUserPen, faUserPlus, faUserSlash,}
...
function EmployeesCom({type}) {
      ...
      fetch(url, {
        ...
        .then((data) => {
          const filteredUsers = data.filter((user) =>
            [user.fullName, user.nationalID, user.roleName, user.phoneNumber, user.departmentName]
              .some((field) =>
    ...
    if (!users) {
      return <LoadingOrError data={users} btnTitle={type === "active" ? "إضافة موظف" : "الموظفين النشطين"} btnLink={type === "active" ? "/add-employee" : "/employees/active"} />;
    }
    ...
    return (
        ...
        <div className="d-flex mb-4 justify-content-between">
            ...
            <div className="d-flex">
                ...
                {type === 'active'? <BtnLink name="إضافة موظف" link="/add-employee" class="m-3 btn btn-primary m-0" />
                    ...
                    <tr key={index}>
                        ...
                        {type === 'active' ? 
                            <td style={{ height: "50px" }}>
                                <Link to={`/profile/user/${user.id}`} className="ms-1"><FontAwesomeIcon icon={faIdCard} color="green" className="fontt" /></Link>
                                <Link to={`/employee/${user.id}`} className="ms-1"><FontAwesomeIcon icon={faUserPen} color="blue" className="fontt" /></Link>
                            ...
                            <td style={{ height: "50px" }}>
                                <Link to={`/profile/user/${user.id}`} className="ms-1"><FontAwesomeIcon icon={faIdCard} color="green" className="fontt" /></Link>
                                <Link to={`/employee/${user.id}/edit`}><FontAwesomeIcon icon={faPenToSquare} color="blue" className="fontt" /></Link>
