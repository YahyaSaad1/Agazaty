...
function PermitLeave() {
    ...
    useEffect(() => {
        const fetchPermitLeaveImage = async () => {
            try {
                const res = await fetch(`${BASE_API_URL}/api/PermitLeave/GetPermitLeaveImageByLeaveId/${permitID}`, {
                    method: 'GET',
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
    ...
    if (!permitLeave) {
        return <p className="text-center text-danger">لم يتم العثور على بيانات التصريح.</p>;
    }
        ...
        <div>
            ...
            <div className="row mt-5 d-flex justify-content-center">
                <div className="col-sm-12 col-lg-10 col-xl-8 box">
                    <div className="d-flex flex-row p-2 align-items-stretch" style={{ direction: 'rtl' }}>
                        <table className="table table-striped m-0 w-100">
                            <tbody>
                                <tr>
                                    <th scope="col">اسم الموظف</th>
                                    <th scope="col" className="text-start">{permitLeave.userName}</th>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <th scope="col" className="text-start">{new Date(permitLeave.date).toLocaleDateString("ar-EG")}</th>
                                </tr>
                                <tr>
                                    <th scope="col">عدد ساعات التصريح</th>
                                    <th scope="col" className="text-start">
                                        {permitLeave.hours.toString().replace(/[0-9]/g, (digit) => '٠١٢٣٤٥٦٧٨٩'[digit])}
                                    </th>
                                </tr>
                            </tbody>
                        </table>
